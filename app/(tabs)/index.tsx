...
const Banner = ({
  ...
  if (settings?.settingsTable[currentWeekDay]?.ticked === false) {
    return (
      <View
        style={style}
        className="bg-white rounded-full py-2 w-[90%] flex flex-row justify-center items-center"
      >
    ...
    currentTime > plan.toJourney.endTime &&
    currentTime > plan.backJourney.endTime
  ) {
    return (
      <View
        style={style}
    ...
    return (
      ...
      >
        <Text>
          Next up: {plan.method} home at{" "}
          {formatTime(plan.backJourney.startTime, settings?.use24hrTime)}!
        </Text>
      </View>
    );
  }
...
export default function HomeScreen() {
  ...
  return (
    <Background plan={plan}>
      ...
      >
        <Banner plan={plan} />
        {settings.settingsTable[currentWeekDay].ticked && (
          <View className="flex flex-col">
            <TripCard
              ...
              start={{
                location: "Home",
                time: formatTime(
                  plan.toJourney.startTime,
                  settings.use24hrTime
                ),
              }}
              end={{
